...
def _generate_benchmark_code(
    ...
    for i, name in enumerate(kernel_names):
        ...
        benchmark_text += f"                {name}_vector_type out;\n"
        benchmark_text += f"                kernel.run(&out);\n"
        benchmark_text += f"                int lanes = vec_lanes_of(out);\n"
        benchmark_text += f"                for (int j = 0; j < lanes && pos + j < N; j++) {{\n"
        benchmark_text += f"                    output_{name}[pos + j] = ((float*)&out)[j];\n"
        benchmark_text += f"                }}\n"
        benchmark_text += f"                pos += lanes;\n"
        benchmark_text += f"            }}\n"
        benchmark_text += f"        }}\n"
