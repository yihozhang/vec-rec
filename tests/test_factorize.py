...
def test_example_1():
    ...
    assert res == [TIKernel([1.0, -3.0], Type.Arith), TIKernel([1.0, -2.0], Type.Arith)]
...
def test_example_2():
    ...
    print(res)
    assert res == [TIKernel([1.0, -3.0], Type.Arith), TIKernel([1.0, -2.0], Type.Arith), TIKernel([1.0, -1.0], Type.Arith)]


def test_example_3():
    # x^2 + 1 => complex roots i and -i => second-order factor x^2 + 1
    res = factorize_polynomial([1, 0, 1])
    assert res == [TIKernel([1.0, 0.0, 1.0], Type.Arith)]


def test_example_4():
    # x^3 - 1 = (x - 1)(x^2 + x + 1)
    res = factorize_polynomial([1, 0, 0, -1])
    assert res == [TIKernel([1.0, -1.0], Type.Arith), TIKernel([1.0, 1.0, 1.0], Type.Arith)]
